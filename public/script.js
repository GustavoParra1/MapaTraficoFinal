...
async function geocodeAddress(address) {
  const serverUrl = `http://localhost:3003/api/geocode?address=${encodeURIComponent(address)}`;
  
  console.log(`🔍 Geocodificando desde servidor: "${address}"`);
  ...
  try {
    ...
    if (data.success && data.lat && data.lng) {
      console.log(`✅ Geocodificado exitosamente: ${data.address}`, { lat: data.lat, lon: data.lng });
      return { lat: data.lat, lon: data.lng };
    } else {
      console.warn(`⚠️ Servidor retornó error: ${data.message}`);
      return null;
    }
  } catch (error) {
    console.error(`❌ Error geocodificando desde servidor: ${error.message}`);
    return null;
  }
}
