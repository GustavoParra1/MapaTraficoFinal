...
auth.onAuthStateChanged((user) => {
  ...
  rtdb.ref("usuarios/" + user.uid).once("value")
    .then((snap) => {
      if (!snap.exists()) {
        throw new Error("Usuario sin perfil en RTDB");
      }
      ...
      // --- AUTO-CREAR PATRULLA (SEGURO) ---
if (perfil.rol === "patrulla" && perfil.patrullaID) {
  console.log("🧪 DB URL:", rtdb.app.options.databaseURL);
  console.log("🧪 Creando patrulla:", perfil.patrullaID);
  ...
  // flags globales patrulla
  window.ES_PATRULLA = true;
  window.PATRULLA_ID = perfil.patrullaID;
}

      // 🔒 BLOQUEO POR ROL
      if (perfil.rol !== "com") {
        alert("🚫 Acceso solo para personal del COM");
        console.warn("Acceso bloqueado. Rol:", perfil.rol);
        return auth.signOut().then(() => {
          window.location.href = "login.html";
        });
      }

      // Variables globales SOLO COM
      window.USER_ROL = "com";
      window.ES_PATRULLA = false;
    ...
    .catch((err) => {
      ...
      alert("Usuario sin permisos configurados");
      auth.signOut().then(() => {
        window.location.href = "login.html";
      });
    });
...
// ============================
function iniciarApp() {
  console.log("🚀 App iniciada con Auth");
  ...
  setTimeout(() => {
    if (window.mymap) {
      ...
      console.log("🔄 Mapa recalculado (invalidateSize)");

    }
  }, 300);

  // iniciarMapaPatrullas(); // (sigue sin usarse)
}
// =======================================================
...
const baseLayers = {
  osm: L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    // Google Mutant layers (only if plugin and Google API are available)
    googleStreets: (typeof L.gridLayer.googleMutant === 'function') ? L.gridLayer.googleMutant({ type: 'roadmap', maxZoom: 21 }) : L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '&copy; OpenStreetMap contributors', maxZoom: 19 }),
    googleSatellite: (typeof L.gridLayer.googleMutant === 'function') ? L.gridLayer.googleMutant({ type: 'satellite', maxZoom: 21 }) : L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '&copy; OpenStreetMap contributors', maxZoom: 19 }),
    googleHybrid: (typeof L.gridLayer.googleMutant === 'function') ? L.gridLayer.googleMutant({ type: 'hybrid', maxZoom: 21 }) : L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '&copy; OpenStreetMap contributors', maxZoom: 19 })
};
...
const heatLayer = L.heatLayer([], { radius: 25 }).addTo(mymap);
...
function updateSiniestrosLayers(dataToDisplay, totalCount, markerColor = null) { // Añadir markerColor como parámetro opcional
    ...
    if (heatmapCheckbox.checked && !mymap.hasLayer(heatLayer)) {
...
function updateTopSiniestrosPanel(rankedLocations) {
    ...
    rankedLocations.forEach((location, index) => {
        ...
        button.addEventListener('click', (e) => {
            ...
            highlightMarker(location, 'siniestro');
...
function applyCamarasFilters() {
    ...
    allCamerasData.forEach(camara => {
        const lat = parseFloat(String(camara.Longitud).replace(',', '.'));
        const lon = parseFloat(String(camara.Latitud).replace(',', '.'));
...
function populateLprSelector() {
    if (!allCamerasData) return;
    const direccionHeaderKey = Object.keys(allCamerasData[0] || {}).find(k => k.toLowerCase().includes('direcci'));
    lprCameras = allCamerasData.filter(camera => camera.LPR || (camera['Cant. Cam. LPR'] && parseInt(camera['Cant. Cam. LPR']) > 0));
    ...
    lprCameras.forEach(camera => {
        ...
        option.textContent = `${camera['N CAMARA']} - ${camera[direccionHeaderKey]}`;
...
function displayLprRadius() {
    loadBaseCSVData().then(() => {
        ...
        const lat = parseFloat(String(selectedLprCamera.Longitud).replace(',', '.'));
        const lon = parseFloat(String(selectedLprCamera.Latitud).replace(',', '.'));
        ...
        allCamerasData.forEach(camera => {
            ...
            const camLat = parseFloat(String(camera.Longitud).replace(',', '.'));
            const camLon = parseFloat(String(camera.Latitud).replace(',', '.'));
...
const roboHeatLayer = L.heatLayer([], { radius: 25 }).addTo(mymap);
...
case 'siniestros_noche': {
  try {
    ...
    // Limpiar capas de consultas anteriores
      ...
      setTimeout(async () => {
        ...
        for (const busLine of uniqueBusLines) {
          ...
          for (const cameraOnLine of camerasOnLine) {
            ...
            if (camaraData) {
              const camLat = parseFloat(String(camaraData.Longitud).replace(',', '.'));
              const camLon = parseFloat(String(camaraData.Latitud).replace(',', '.'));
              if (!isNaN(camLat) && !isNaN(camLon)) {
                const cameraIcon = L.divIcon({
    ...
    case 'lineas_colectivo_siniestros': {
      ...
      setTimeout(async () => {
        ...
        if (hotspotCameras.length === 0) {
            resultadosDiv.innerHTML = '✅ No se encontraron cámaras con más de 10 siniestros en sus cercanías.';
            return;
        }
...
async function mostrarCoberturaEscolar() {
    ...
    colegiosData.features.forEach(feature => {
        ...
        for (const camara of allCamerasData) {
            const camLat = parseFloat(String(camara.Longitud).replace(',', '.'));
            const camLon = parseFloat(String(camara.Latitud).replace(',', '.'));
            if (isNaN(camLat) || isNaN(camLon)) continue;
...
barriosData.features.forEach(barrio => {
    ...
    const porcentaje = ((sinCobertura / total) * 100).toFixed(1);

    resultados.push({
        Barrio: nombreBarrio,
        "Colegios totales": total,
        "Sin cobertura": sinCobertura,
        "% sin cobertura": porcentaje
    });
    ...
    icon: L.divIcon({
        ...
        html: `
            <div class="colegio-label-box">
                ...
                <span class="${porcentaje > 50 ? 'alto' : porcentaje > 20 ? 'medio' : 'bajo'}">
                    ${(100 - porcentaje).toFixed(1)}% con cobertura
                </span>
            </div>
        `
    })
...
function verCamaraPatrulla(movil) {
  // ⛔ SI SE CERRÓ MANUALMENTE, NO REABRIR
  if (visorCerradoManual) {
    console.log("🚫 Cámara bloqueada por cierre manual");
    return;
  }

  console.log("📹 [HLS] Abriendo stream HLS patrulla:", movil);

  const visor = document.getElementById("visor-patrulla");
  const video = document.getElementById("visorVideo");
  const nombreVisor = document.getElementById("nombre-patrulla-visor");

  // Mostrar visor y actualizar nombre
  if (visor) visor.classList.remove("visor-hidden");
  if (nombreVisor) nombreVisor.textContent = movil;
  if (visor) visor.offsetHeight; // Forzar reflow

  // URL del stream HLS con timestamp para evitar caché
  const hlsUrl = `/streams/${movil}/stream.m3u8?t=${Date.now()}`;
  
  console.log(`📹 [HLS] URL del stream: ${hlsUrl}`);

  if (!video) {
    console.error("❌ [HLS] Elemento de video no encontrado");
    return;
  }
  ...
  if (typeof Hls !== 'undefined' && Hls.isSupported()) {
    console.log("✅ [HLS] HLS.js disponible, usando para reproducción");
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, () => {
      console.log("✅ [HLS] Manifest parseado, iniciando reproducción");
      video.play().catch(err => {
        console.error("⚠️ [HLS] Error al reproducir:", err);
      });
    });

    hls.on(Hls.Events.ERROR, (event, data) => {
      console.error("❌ [HLS] Error HLS:", event, data);
      try { console.dir(data); } catch (e) { /* ignore */ }

      // Detectar errores específicos
      if (data && (data.details === "fragParsingError" || data.reason === "Found no media in fragment")) {
        console.warn("⚠️  [HLS] El video no tiene datos válidos");
        console.warn("📝 Por favor, genera un video HLS válido:");
        console.warn("   1. Abre PowerShell como Administrador");
        console.warn("   2. Instala FFmpeg: winget install ffmpeg");
        console.warn("   3. Ejecuta: generar_hls_admin.bat");
        console.warn("   4. Recarga el navegador");
      }

      if (data && data.fatal) {
        console.error("❌ [HLS] Error fatal detectado, intentando recuperar");
        try {
          hls.recoverMediaError();
        } catch (err) {
          console.error('❌ [HLS] recoverMediaError fallo:', err);
        }

        // Si tras intentar recuperar sigue fallando, destruir la instancia y notificar al usuario
        setTimeout(() => {
          if (window.hlsInstance) {
            console.error('❌ [HLS] Persisten errores tras recover, destruyendo instancia HLS');
            try { window.hlsInstance.destroy(); } catch (err) { console.error('Error destruyendo HLS instance:', err); }
            window.hlsInstance = null;
            const msg = 'No se pudo reproducir el stream HLS. Revisá la consola (Network y Logs) y los archivos .m3u8/.ts en el servidor.';
    ...
    console.log("✅ [HLS] Usando soporte nativo de HLS");
    video.src = hlsUrl;
    video.play().catch(err => {
      console.error("⚠️ [HLS] Error al reproducir:", err);
    });
  } else {
    console.error("❌ [HLS] Navegador no soporta HLS");
  }
}
...
function cerrarVisor(e) {
  ...
  // 3. Limpiar HLS
  if (window.hlsInstance) {
    console.log("📹 Destruyendo instancia HLS");
    window.hlsInstance.destroy();
    window.hlsInstance = null;
  }
